...
void productor(){
    ...
    while(item != '*'){
        ...
        if((item>=48 && item<=57) || (item>=65 && item<=90) || (item>=97 && item<=122) || item=='*'){
            // Se tiene que implementar una cola LIFO para el buzón 'almacén1'. Para ello se emplea la prioridad:
            // Si todos los mensajes tienen la misma prioridad, se consumirá de más antiguo a más reciente. 
            // Si tienen prioridad distinta (será este caso), se consumirán primero los de mayor prioridad, 
            // independientemente de su antigüedad en el buzón.
            // Para ello, se obtienen los atributos del buzon 'almacen1' para poder consultar el número de mensajes
            // que contiene en ese momento. Se emplea este valor (de 0 a 4) como prioridad de envío: 
            // el último item insertado siempre será el de mayor prioridad, por lo que será consumido antes que 
            // items de menor prioridad que han sido insertados antes que él
            mq_getattr(almacen1, &attr);
            prioridad= attr.mq_curmsgs;
            printf("Item producido: %c  |  Prioridad: %u\n", item, prioridad);
            
            // Se procede a enviar el item. Se pasa el número de mensajes contenidos en el buzón como valor de prioridad
            mq_send(almacen1, &item, sizeof(char), prioridad);

            // Se consume un item vacío del buzón 'almacen2'
            mq_receive(almacen2, &aviso, sizeof(char), &prioridad);
        }
    }
