...
void recibirMensaje(char* item) {
    // Cola LIFO, se recibe del buzón 'almacen1' el item del productor. Si todos tienen la misma prioridad,
    // se consume de más antiguo a más reciente. Si tienen prioridad distinta (será este caso), se consumirán
    // primero los de mayor prioridad, independientemente de su antigüedad en el buzón
...
void consumidor() {
    ...
    // Se inicia el bucle de consumo, que se detendrá tras consumir todos los item restantes del buzón
    // cuando reciba un asterisco '*' como item
    while(1) {
